...
namespace SearcherLibrary
{
    ...
    public class SearchOtherExtensions
    {
        ...
        public List<MatchedLine> SearchFileForMatch(string fileName, IEnumerable<string> searchTerms, Matcher matcher = null)
        {
            ...
            switch (fileExtension)
            {
                ...
                case ".DOCM":
                    ...
                    break;
                case ".ODP":
                    matchedLines = this.GetMatchesInOdp(fileName, searchTerms);
                    break;
                case ".ODS":
                    matchedLines = this.GetMatchesInOds(fileName, searchTerms);
                    break;
                case ".ODT":
                    matchedLines = this.GetMatchesInOdt(fileName, searchTerms);
                    break;
                case ".PDF":
        ...
        /// <param name="searchTerms">The terms to search.</param>
        /// <returns>The matched lines containing the search terms.</returns>
        private List<MatchedLine> GetMatchesFromOdsContentXml(string fileName, string extractedContentFile, IEnumerable<string> searchTerms)
        {
            int matchCounter = 0;
            List<MatchedLine> matchedLines = new List<MatchedLine>();
        ...
        /// <param name="searchTerms">The terms to search.</param>
        /// <returns>The matched lines containing the search terms.</returns>
        private List<MatchedLine> GetMatchesInOdp(string fileName, IEnumerable<string> searchTerms)
        {
            List<MatchedLine> matchedLines = new List<MatchedLine>();
            int matchCounter = 0;
                                ...
                                try
                                {
                                    ...
                                    foreach (XElement element in XDocument.Load(fullFilePath, LoadOptions.None).Descendants().Where(d => d.Name.LocalName == "page"))
                                    {
                                        ...
                                        if (int.TryParse(slideName.Replace("Slide", string.Empty), out slideNumber))
                                        {
                                            ...
                                            foreach (string searchTerm in searchTerms)
                                            {
                                                MatchCollection matches = Regex.Matches(slideAllText, searchTerm, this.RegexOptions);            // Use this match for getting the locations of the match.
        ...
        /// <summary>
        /// Search for matches in ODS files.
        /// </summary>
        /// <param name="fileName">The name of the file.</param>
        /// <param name="searchTerms">The terms to search.</param>
        /// <returns>The matched lines containing the search terms.</returns>
        private List<MatchedLine> GetMatchesInOds(string fileName, IEnumerable<string> searchTerms)
        {
            List<MatchedLine> matchedLines = new List<MatchedLine>();
            string tempDirPath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, fileName + TempExtractDirectoryName);
                        ...
                        if (!reader.Entry.IsDirectory && reader.Entry.Key == "content.xml")
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(reader.Entry.LinkTarget) && !reader.Entry.Key.Any(c => DisallowedCharactersByOperatingSystem.Any(dc => dc == c)))
                            {
                                try
                                {
                                    reader.WriteEntryToDirectory(tempDirPath, new ExtractionOptions() { ExtractFullPath = true, Overwrite = true });
                                    string fullFilePath = System.IO.Path.Combine(tempDirPath, reader.Entry.Key.Replace(@"/", @"\"));
                                    matchedLines = this.GetMatchesFromOdsContentXml(fileName, fullFilePath, searchTerms);
                                }
                                catch (PathTooLongException ptlex)
                                {
                                    throw new PathTooLongException(string.Format("{0} {1} {2} {3} - {4}", Resources.Strings.ErrorAccessingEntry, reader.Entry.Key, Resources.Strings.InArchive, fileName, ptlex.Message));
        ...
        /// <param name="searchTerms">The terms to search.</param>
        /// <returns>The matched lines containing the search terms.</returns>
        private List<MatchedLine> GetMatchesInOdt(string fileName, IEnumerable<string> searchTerms)
        {
            List<MatchedLine> matchedLines = new List<MatchedLine>();
            string tempDirPath = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, fileName + TempExtractDirectoryName);
                        ...
                        if (!reader.Entry.IsDirectory && reader.Entry.Key == "content.xml")
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(reader.Entry.LinkTarget) && !reader.Entry.Key.Any(c => DisallowedCharactersByOperatingSystem.Any(dc => dc == c)))
                            {
                                try
                                {
                                    ...
                                    matchedLines = this.localMatcherObj.GetMatch(content, searchTerms);
                                }
                                catch (PathTooLongException ptlex)
                                {
                                    throw new PathTooLongException(string.Format("{0} {1} {2} {3} - {4}", Resources.Strings.ErrorAccessingEntry, reader.Entry.Key, Resources.Strings.InArchive, fileName, ptlex.Message));
